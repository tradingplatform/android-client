...
public class OneMonthFragment extends Fragment implements CompoundButton.OnCheckedChangeListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (this.orientation == Configuration.ORIENTATION_PORTRAIT) {
            root = inflater.inflate(R.layout.fragment_one_month, container, false);
        } else {
            root = inflater.inflate(R.layout.fragment_one_month, container, false);
        }
...
//        title.setTypeface(serifLight);
    ...
    private void prepareChart() {
        if (dowJonesData == null) {
            ...
            ArrayList<DataClass> filtered = new ArrayList<DataClass>();
            ...
            for (DataClass article : dowJonesData) {
                if (article.date.getTime().compareTo(date) > 0) {
                    filtered.add(article);
                }
            }

            dowJonesData = filtered;
        ...
        horizontalAxis.setMajorTickInterval(7);
        horizontalAxis.setDateTimeFormat(new SimpleDateFormat("MMM dd"));
